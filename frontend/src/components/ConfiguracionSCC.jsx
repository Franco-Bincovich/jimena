...
export function SheetsSection({ sheetsTab, setSheetsTab, sheetId, setSheetId, sheetVerifyStatus, setSheetVerifyStatus, verifyingSheet, handleVerifySheet, creatingSheet, handleCreateSheet, createdSheetId, createdSheetUrl, googleConnected }) {
  return (
    <div className="p-5 rounded-lg" style={{ backgroundColor: 'var(--c-surface)', border: '0.5px solid var(--c-border)' }}>
      ...
      <div className="flex mb-5" style={{ borderBottom: '0.5px solid var(--c-border)' }}>
      ...
      {sheetsTab === 'existing' ? (
        <div>
          <InstruccionBox>
            <ol className="list-none m-0 p-0 flex flex-col gap-1">
              ...
              <li className="pl-3"><code className="font-mono text-[11.5px] text-muted-dark">docs.google.com/spreadsheets/d/<span style={{ color: '#FF6B00', fontWeight: 600 }}>[ESTE-ES-EL-ID]</span>/edit</code></li>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleVerifySheet} disabled={verifyingSheet || !sheetId.trim()} className="flex items-center gap-2 px-4 py-2 text-muted hover:text-text text-[12.5px] rounded-md transition-colors min-h-[36px] disabled:opacity-50 flex-shrink-0" style={{ border: '0.5px solid var(--c-border-l)' }}>
        ...
        <div>
          ...
          {createdSheetId ? (
            <div className="flex items-center justify-between px-3 py-2.5 rounded-md" style={{ backgroundColor: 'var(--c-ok-bg)', border: '0.5px solid var(--c-ok-border)' }}>
              <div>
                <p className="text-[12px] text-muted-dark mb-0.5">Sheet creado</p>
                <code className="text-[12px] font-mono" style={{ color: 'var(--c-ok-text)' }}>{createdSheetId}</code>
              </div>
              {createdSheetUrl && <a href={createdSheetUrl} target="_blank" rel="noopener noreferrer" className="text-[12px] underline transition-opacity hover:opacity-80" style={{ color: 'var(--c-ok-text)' }}>Abrir Sheet ↗</a>}
            </div>
          ) : (
      ...
      <div className="mt-5 rounded-md p-3" style={{ backgroundColor: 'var(--c-bg)', border: '0.5px solid var(--c-border-s)' }}>
        <p className="text-muted-dark text-[11px] uppercase tracking-wider mb-2">Columnas que se van a registrar</p>
        <div className="flex flex-wrap gap-1.5">
          {SHEET_COLS.map((col) => (
            <span key={col} className="px-2 py-0.5 rounded text-[11px] font-mono" style={{ backgroundColor: 'var(--c-border-s)', color: 'var(--c-muted-d)', border: '0.5px solid var(--c-border)' }}>{col}</span>
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="p-5 rounded-lg" style={{ backgroundColor: 'var(--c-surface)', border: '0.5px solid var(--c-border)' }}>
      ...
      {contactosCC.length > 0 && (
        <div className="rounded-md mb-4 overflow-hidden" style={{ border: '0.5px solid var(--c-border-l)' }}>
          {contactosCC.map((c, i) => (
            <div key={c.id} className="flex items-center justify-between px-3 py-2.5" style={{ borderTop: i > 0 ? '0.5px solid var(--c-border)' : undefined, backgroundColor: 'var(--c-bg)' }}>
              ...
              <button type="button" onClick={() => handleDeleteCC(c.id)} disabled={deletingCC === c.id} className="p-1.5 text-muted hover:text-error transition-colors disabled:opacity-40 flex-shrink-0 ml-3" title="Eliminar">
                {deletingCC === c.id ? (
                  <div className="w-3 h-3 rounded-full border animate-spin" style={{ borderColor: 'var(--c-border-l)', borderTopColor: 'var(--c-muted)' }} />
                ) : (
      ...
      <button type="button" onClick={() => setCcModalOpen(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-muted hover:text-text text-[12px] rounded-md transition-colors" style={{ border: '0.5px solid var(--c-border-l)' }}>+ Agregar contacto</button>
    </div>
  )
}
...
export function ContactosCCModal({ ccModalOpen, savingCC, ccNombre, setCcNombre, ccEmail, setCcEmail, handleAddCC, setCcModalOpen }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }} onClick={(e) => { if (e.target === e.currentTarget && !savingCC) setCcModalOpen(false) }}>
      <div className="w-full max-w-sm rounded-lg p-5 flex flex-col gap-4" style={{ backgroundColor: 'var(--c-surface)', border: '0.5px solid var(--c-border-l)' }}>
        ...
        <div className="flex justify-end gap-2.5">
          <button type="button" onClick={() => { setCcModalOpen(false); setCcNombre(''); setCcEmail('') }} disabled={savingCC} className="px-3 py-1.5 text-muted hover:text-text text-[12.5px] rounded-md transition-colors disabled:opacity-50" style={{ border: '0.5px solid var(--c-border-l)' }}>Cancelar</button>
