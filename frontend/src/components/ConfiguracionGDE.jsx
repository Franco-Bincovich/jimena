    ...
    <div className="p-5 rounded-lg" style={{ backgroundColor: 'var(--c-surface)', border: '0.5px solid var(--c-border)' }}>
      ...
      {googleConnected ? (
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setDisconnectOpen(true)} className="px-3 py-1.5 text-muted hover:text-error text-[12px] rounded-md transition-colors min-h-[36px]" style={{ border: '0.5px solid var(--c-border-l)' }}>Desconectar</button>
        </div>
      ) : googleNotConfigured ? (
        <div className="flex items-start gap-2.5 px-3 py-3 rounded-md text-[12.5px]" style={{ backgroundColor: 'var(--c-warn-bg)', border: '0.5px solid var(--c-warn-border)', color: 'var(--c-warn-text)' }}>
        ...
        <div>
          ...
          <div className="flex justify-center">
            <button onClick={handleConnectGoogle} disabled={connectingGoogle} className="flex items-center justify-center gap-2.5 py-2.5 rounded-lg transition-colors disabled:opacity-60 min-h-[44px]" style={{ width: '260px', backgroundColor: '#ffffff', color: '#111111', border: '1px solid #ccc', fontSize: '13px', fontWeight: 500 }}>
    ...
    <div className="p-5 rounded-lg" style={{ backgroundColor: 'var(--c-surface)', border: '0.5px solid var(--c-border)' }}>
      ...
      <InstruccionBox>
        <ol className="list-none m-0 p-0 flex flex-col gap-1">
          ...
          <li><span style={{ color: '#FF6B00' }}>2.</span> Creá una carpeta nueva llamada <code className="font-mono text-text">Facturas</code></li>
          <li><span style={{ color: '#FF6B00' }}>3.</span> Abrí esa carpeta y copiá el ID que aparece en la URL:</li>
          <li className="pl-3"><code className="font-mono text-[11.5px] text-muted-dark">drive.google.com/drive/folders/<span style={{ color: '#FF6B00', fontWeight: 600 }}>[ESTE-ES-EL-ID]</span></code></li>
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handleVerifyDrive} disabled={verifyingDrive || !driveFolderId.trim()} className="flex items-center gap-2 px-4 py-2 text-muted hover:text-text text-[12.5px] rounded-md transition-colors min-h-[36px] disabled:opacity-50 flex-shrink-0" style={{ border: '0.5px solid var(--c-border-l)' }}>
      ...
      <div className="rounded-md p-3" style={{ backgroundColor: 'var(--c-bg)', border: '0.5px solid var(--c-border-s)' }}>
        <p className="text-muted-dark text-[11px] uppercase tracking-wider mb-2">Estructura de carpetas que se va a crear</p>
        <pre className="text-[12px] m-0 font-mono leading-6 text-muted-dark">{`📁 Facturas/\n   📁 {nombre del proveedor}/`}</pre>
      </div>
    </div>
  )
}
